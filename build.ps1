# Script that builds libbfoverlay
#
# Version: 20260608
...
Else
{
	...
	If (-Not (Test-Path "${VSSolutionPath}"))
	{
		...
		Invoke-Expression -Command "& '${Python}' ${MSVSCppConvert} --output-format ${VisualStudioVersion} ${VSToolsOptions} msvscpp\libbfoverlay.sln 2>&1" | %{ "$_" }
	}
}
If (${VisualStudioVersion} -eq "2026")
{
	$VSSolutionFile = "${VSSolutionPath}\libbfoverlay.slnx"
}
Else
{
	$VSSolutionFile = "${VSSolutionPath}\libbfoverlay.sln"
}
