...
#include "../libbfoverlay/libbfoverlay_range.h"

#if defined( __GNUC__ ) && !defined( LIBBFOVERLAY_DLL_IMPORT )

/* Tests the libbfoverlay_range_initialize function
 * Returns 1 if successful or 0 if not
 */
int bfoverlay_test_range_initialize(
     void )
{
	...
	/* Test regular cases
	 */
	result = libbfoverlay_range_initialize(
	          &range,
	          &error );
	...
	result = libbfoverlay_range_free(
	          &range,
	          &error );
	...
	range = (libbfoverlay_range_t *) 0x12345678UL;
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libbfoverlay_range_initialize with malloc failing
		 */
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libbfoverlay_range_initialize with memset failing
		 */
...
/* Tests the libbfoverlay_range_free function
 * Returns 1 if successful or 0 if not
 */
int bfoverlay_test_range_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
#endif
{
	...
	BFOVERLAY_TEST_RUN(
	 "libbfoverlay_range_initialize",
	 bfoverlay_test_range_initialize );

	BFOVERLAY_TEST_RUN(
	 "libbfoverlay_range_free",
	 bfoverlay_test_range_free );

	/* TODO add tests for libbfoverlay_range_compare */
